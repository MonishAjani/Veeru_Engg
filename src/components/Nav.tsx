...
export default function Nav() {
  ...
  return (
    <header className="relative z-50 bg-white w-full shadow-sm border-b border-gray-100">
      ...
      {/* Mobile menu */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg border-t border-gray-100 z-40">
          <nav className="flex flex-col px-4 pb-4 pt-2">
            {links.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="text-gray-700 py-3.5 text-base font-medium border-b border-gray-100"
            ...
            <Link
              href="/contact"
              className="text-white py-3 mt-3 flex justify-center bg-orange-500 rounded-lg"
              onClick={() => setIsMenuOpen(false)}
            >
              Contact Us
            </Link>
